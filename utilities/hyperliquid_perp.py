...
class PerpBitget:
    ...
    async def get_open_positions(self, pairs) -> List[Position]:
        ...
        for position in resp:
            ...
            take_profit_price = position.get("takeProfitPrice", 0)
            stop_loss_price = position.get("stopLossPrice", 0)
    ...
    async def place_order(
        ...
        try:
            ...
            margin_mode_param = "cross" if margin_mode == "crossed" else "isolated"
            ...
            order_id = resp["id"]
            pair_ext = self.pair_to_ext_pair(resp["symbol"])
            order = await self.get_order_by_id(order_id, pair_ext)
            return order
